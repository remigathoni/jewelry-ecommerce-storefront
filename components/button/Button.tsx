...
export default function Button({text, background, handler}:{text:string, background:string, handler?:any}) {
  return (
    <button className={styles.button} aria-label="Shop now" style={{backgroundColor: `${background}`}} onClick={handler}>
      <span>{text}</span>
      <div>
